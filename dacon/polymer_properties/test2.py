...
#         img.save(path + f'train_imgs/{file}.png')
...
imgs = (path + 'train_imgs/'+train.uid+'.png').to_numpy()

from sklearn.utils import shuffle
imgs, seqs, labels = shuffle(imgs, seqs, labels, random_state=21)
...
for idx, row in tqdm(test.iterrows()):
    ...
    if m != None:
        img = Draw.MolToImage(m, size=(300,300))
        img.save(path + f'test_imgs/{file}.png')

test_seqs = tokenizer.txt2seq(test.SMILES)
test_imgs = (path + 'test_imgs/'+test.uid+'.png').to_numpy()
